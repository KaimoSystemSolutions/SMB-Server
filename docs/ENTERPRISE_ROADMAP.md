...
### M10.3 — SMB compression ✅ (LZ77 + LZNT1; LZ77+Huffman codec is a documented follow-up)
...
- [x] **LZNT1** (MS-XCA §2.5) codec — `Lznt1.Compress`/`Decompress`: chunk-oriented (≤4096-byte chunks,
      16-bit header = compressed-flag ‖ signature `0b011` ‖ size−1), flag-group tokens with the
      **position-dependent** length/displacement split (12/4 bits, shifting toward displacement as the
      chunk output passes 16/32/64/…). The split follows the spec exactly so a stock Windows compressor's
      output decodes and ours decodes on Windows (cross-checked against two independent decoders). Opt in
      via `CompressionPreference = [Lznt1, …]` — the default preference stays LZ77.
...
- [x] **LZ77+Huffman** (MS-XCA §2.2) **decoder** — `Lz77Huffman.Decompress`: 256-byte code-length table →
      canonical decode table, 32-bit register read in 16-bit LE units, match symbols 256–511 with interleaved
      extra length bytes, multi-block (each ≤64 KiB). Lets the server **receive** LZ77+Huffman frames (e.g.
      from Windows). Capability is now split: `SmbCompressor.DecodableAlgorithms` (advertised + decodable =
      LZ77/LZNT1/LZ77+Huffman) vs `EncodableAlgorithms` (produced = LZ77/LZNT1). `NegotiateProcessor` advertises
      decodables but picks the outbound algorithm from encodables, so a decode-only algorithm is received yet
      never emitted. Validated by a hand-built, spec-derived frame (independent of any encoder).

> **LZ77+Huffman encoder follow-up:** the **decoder** ships (decode-only); the server can receive but not
> produce LZ77+Huffman. Only the **encoder** remains — its interleaved length bytes and end-of-block 16-bit
> flush must be byte-exact, and that is un-validatable without a live Windows decode, so producing it is
> deferred rather than risk emitting a frame Windows rejects. It slots into `EncodableAlgorithms` +
> `SmbCompressor.CompressPayload` when added (same honesty stance as the Kerberos ticket-crypto and LDAP
...
| 2026-07-08 | Phase 10 / M10.3 | Complete (LZ77 + LZNT1 symmetric; LZ77+Huffman decode-only) | SMB compression. Pure `Smb.Protocol.Compression`: `PlainLz77` (MS-XCA §2.4, 8 KiB window, capped-match spec-safe output), `CompressionTransformHeader`/`CompressionPayloadHeader` (unchained §2.2.42.1 + chained §2.2.42.2 decode incl. None/Pattern_V1), `SmbCompressor` orchestrator. `NegotiateProcessor` intersects client list ∩ server preference ∩ codecs, stores `SmbConnection.CompressionAlgorithm`, echoes context; gated on `EnableCompression` (off by default, 3.1.1 only, unchained). Host (`SmbConnectionHandler`) compresses responses ≥ `CompressionMinSize` (4096) when not encrypting and decodes inbound compression frames (`EncodeOutbound`/`DecodeInboundFrame`). Options `EnableCompression`/`CompressionPreference`/`CompressionMinSize` + fluent `SmbServerBuilder.UseCompression`. 21 tests (`Phase10CompressionTests.cs`), full suite 383 → **404 green**. Follow-up (post-roadmap): full **LZNT1** codec (encode+decode, MS-XCA §2.5) and **LZ77+Huffman** decoder (MS-XCA §2.2) added; capability split into `SmbCompressor.DecodableAlgorithms` (advertised: LZ77/LZNT1/LZ77+Huffman) vs `EncodableAlgorithms` (produced: LZ77/LZNT1) so LZ77+Huffman is received-but-never-emitted. Huffman multi-block boundary corrected (`BitReader.NextTablePosition` backs out the 16-bit look-ahead over-read). Hand-built spec vectors added for the LZNT1 compressed-chunk decode and the two-block Huffman boundary. LZ77+Huffman **encoder** still deferred (byte-exact output un-validatable without a live Windows decode). |
