...
| **Phase 10 Transport hardening** ✅ (**M10.1 SMB over TLS** — `SslStream` wrapper via `SmbServerBuilder.UseTls(cert)`, mutual TLS; **M10.2 SMB over QUIC** — `SmbServerBuilder.UseQuic(cert)`, TLS 1.3 + ALPN "smb"; **M10.3 compression** — LZ77 + LZNT1 via `UseCompression`, negotiate context) | ✅ |
